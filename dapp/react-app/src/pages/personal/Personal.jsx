...
const Personal = () => {
  ...
  async function getStakes() {
    ...
    if (provider) {
      ...
      const events = await stakingFactoryContract.getPastEvents('Staked', {
      ...
      const createEvents = await stakingFactoryContract.getPastEvents('CreateStakeContract', {
      ...
      for (let event of createEvents) {
        ...
        let lockDurationIndex = event.returnValues.lockDuration;
        let poolAddress = event.returnValues.stakePool
  ...
  const handleUnstake = async (poolAddress) => {
    if (provider) {
      ...
      try {
        let timestamp = await poolContract.methods.expirationTimestamp().call()
        let now = Date.parse(new Date()) / 1000
        if (now < timestamp) {
          toast.info("Staking period is still active")
          return;
        }
        ...
        let stakeInfo = await poolContract.methods.userinfo(currentAccount).call()
        if (stakeInfo.amount == 0) {
          toast.error("Already claimed")
          return
        }
        poolContract.once('GetProfit', { filter: { creator: currentAccount }, fromBlock: 'latest' }, (error, event) => {
          if (error) console.error(error);
          else {
            const pofit = event.returnValues.amount;  // 根据你的事件定义获取相关数据
            const share = web3.utils.fromWei(pofit, 'ether')
            toast.success(`You've earned ${share} BAT as your basic staking reward.`);
          }
        });
        poolContract.once('Unstaked', { filter: { creator: currentAccount }, fromBlock: 'latest' }, (error, event) => {
          if (error) console.error(error);
          else {
            const pofit = event.returnValues.amount;  // 根据你的事件定义获取相关数据
            const share = web3.utils.fromWei(pofit, 'ether')
            toast.success(`You've earned ${share} BAT as your staking reward.`);
          }
        });
        await poolContract.methods.unstake().send({ from: currentAccount });
  ...
  async function ArbitrationEvent(url) {
    ...
    const targetUrl = `${origin}/beta/init?attachUrl=${encodeURIComponent(url)}`;
  ...
  const fetchBannedUrls = async () => {
    try {
      ...
      // 将数据转换为适当的格式
      console.log("test")
      const rows = data.map((item, index) => createData(index + 1, item.url, item.downvotes, item.upvotes, '10%', '3 days', item.pooladdress));
      console.log(rows)
      setRows(rows)
      //setTableData(formattedData);
      // 计算总的 upvotes 和 downvotes
      const totalUp = data.reduce((acc, item) => acc + parseInt(item.upvotes), 0);
      const totalDown = data.reduce((acc, item) => acc + parseInt(item.downvotes), 0);
      setTotalUpvotes(parseInt(totalUp));
      setTotalDownvotes(parseInt(totalDown));
    } catch (error) {
  ...
  return (
    <div className='page'>
      ...
      <div className='personal-wrap'>
        <div className='up'>
          <div className='target-data'>{t("Personal Stake")}</div>
          <div className='data-word'>
            ...
            <div className='flex-row'>
              <div className="data-name">{t("Total Token Stake")}</div>
              ...
              <Table sx={{ minWidth: '10vw' }} size="small" aria-label="a-dense-table">
                <TableHead>
                  <TableRow>
                    ...
                    <StyledTableCell align="left">{t("staking period")}</StyledTableCell>
