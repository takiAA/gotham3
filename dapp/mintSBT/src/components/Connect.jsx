...
const Connect = ({ onProviderUpdate }) => {
  ...
  function checkChainId(chainId,toWarn) {
    if (parseInt(chainId, 16) != parseInt(CHAIN_ID)) {
      if (toWarn){
        toast.warn("Gotham3 only supports the Scroll Testnet.");
      }
  ...
  const disconnectMetamask = async () => {
    ...
    return (
        <div>
            {address ? (
                <div className='connect-button' onClick={disconnectMetamask}>{address.slice(0, 6)}...{address.slice(38, 42)}</div>
            )
                : (
                    <div className="connect-button" onClick={connectMetamask}>
                        CONNECT
                    </div>
                )}
        </div>
